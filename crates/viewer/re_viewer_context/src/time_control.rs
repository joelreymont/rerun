...
/// Bundles timeline metadata and their histograms together for convenience.
///
/// These two data structures are often passed together, so this struct
/// provides a convenient way to handle them as a single unit with helper methods.
pub struct TimelineTimes<'a> {
    pub timelines: &'a BTreeMap<TimelineName, Timeline>,
    pub time_histogram_per_timeline: &'a TimeHistogramPerTimeline,
}

impl<'a> TimelineTimes<'a> {
    pub fn new(
        timelines: &'a BTreeMap<TimelineName, Timeline>,
        time_histogram_per_timeline: &'a TimeHistogramPerTimeline,
    ) -> Self {
        Self {
            timelines,
    ...
    /// Check if a timeline exists in the histogram.
    pub fn has_timeline(&self, timeline_name: &TimelineName) -> bool {
        self.time_histogram_per_timeline.has_timeline(timeline_name)
    }

    /// Get the histogram for a specific timeline.
    pub fn get_histogram(&self, timeline_name: &TimelineName) -> Option<&TimeHistogram> {
        self.time_histogram_per_timeline.get(timeline_name)
    }

    /// Get the timeline metadata.
    pub fn get_timeline(&self, timeline_name: &TimelineName) -> Option<&Timeline> {
        self.timelines.get(timeline_name)
    }

    /// Iterate over all timelines with their histograms.
    pub fn iter(&self) -> impl Iterator<Item = (&TimelineName, &Timeline, &TimeHistogram)> {
        self.time_histogram_per_timeline.iter().filter_map(|(name, hist)| {
            self.timelines.get(name).map(|timeline| (name, timeline, hist))
        })
    }
}
...
impl TimeControl {
    ...
    pub fn handle_time_commands(
        &mut self,
        blueprint_ctx: Option<&impl BlueprintContext>,
        timeline_times: &TimelineTimes<'_>,
    ...
    fn handle_time_command(
        ...
        match command {
            ...
            TimeControlCommand::SetActiveTimeline(timeline_name) => {
                ...
                if let Some(timeline) = timeline_times.get_timeline(timeline_name) {
                    self.timeline = ActiveTimeline::UserEdited(*timeline);
                } else {
                    self.timeline = ActiveTimeline::Pending(Timeline::new_sequence(*timeline_name));
                }

                if let Some(full_valid_range) = self.full_valid_range(timeline_times.time_histogram_per_timeline)
            ...
            TimeControlCommand::SetLoopMode(loop_mode) => {
                if self.loop_mode != *loop_mode {
                    ...
                    if self.loop_mode != LoopMode::Off {
                        if self.play_state() == PlayState::Following {
                            self.set_play_state(
                                timeline_times.time_histogram_per_timeline,
                                PlayState::Playing,
                                blueprint_ctx,
                            );
                        }
            ...
            TimeControlCommand::SetPlayState(play_state) => {
                if self.play_state() != *play_state {
                    self.set_play_state(timeline_times.time_histogram_per_timeline, *play_state, blueprint_ctx);
            ...
            TimeControlCommand::TogglePlayPause => {
                self.toggle_play_pause(timeline_times, blueprint_ctx);

                NeedsRepaint::Yes
            }
            TimeControlCommand::StepTimeBack => {
                self.step_time_back(timeline_times.time_histogram_per_timeline, blueprint_ctx);

                NeedsRepaint::Yes
            }
            TimeControlCommand::StepTimeForward => {
                self.step_time_fwd(timeline_times.time_histogram_per_timeline, blueprint_ctx);
    ...
    fn toggle_play_pause(
        ...
        if self.playing {
            ...
            // Start from beginning if we are at the end:
            if let Some(hist) = timeline_times.get_histogram(self.timeline.name())
...
fn step_back_time(time: TimeReal, hist: &TimeHistogram) -> TimeInt {
    hist.prev_key(time.ceil().as_i64())
        .map(TimeInt::new_temporal)
