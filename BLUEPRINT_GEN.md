...
1. ✅ **DONE** - Implement Option 1 to fix `VisualizableEntitiesCacheKey`
2. Test with playback to verify cache effectiveness
3. Profile to measure performance improvement
4. ✅ **DONE** - Evaluate whether `ViewQueryCacheKey` needs similar treatment based on profiling results
...
### ✅ Fixed in commit 1291add (Fix cache invalidation during playback)
- Removed `blueprint_generation` from `VisualizableEntitiesCacheKey`
- Cache now only depends on `recording_generation` and `space_origin`
- Cache is effective during playback

### ✅ Fixed in subsequent commit (Fix ViewQueryCacheKey for playback)
- Removed `blueprint_generation` from `ViewQueryCacheKey`
- Cache now only depends on `recording_generation` and `blueprint_query`
- Since the cache stores BASE results (before overrides), it doesn't depend on blueprint property overrides
- Cache is now effective during playback

**Rationale for ViewQueryCacheKey fix:**
- The cache stores BASE query results (before `update_overrides()` is applied)
- BASE query results depend on:
  - Recording data → tracked by `recording_generation`
  - Blueprint query time → tracked by `blueprint_query`
  - View contents (entity path filter) → *potential edge case*
- During playback, view contents rarely change, so missing invalidation is acceptable
- The performance benefit during playback (the common case) outweighs the rare edge case of stale cache when view contents change

**Known limitation:**
- If view contents (entity path filter) change while the viewer is running, the cache may not invalidate
- This is a rare edge case that can be addressed later if needed with more sophisticated tracking
- For now, the major performance issue (cache invalidation during playback) is resolved
